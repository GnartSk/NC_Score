...
const UploadIcsButton = () => {
            ...
            if (response.ok) {
                ...
                try {
                    ...
                    if (htmlScoreData) {
                        ...
                        const allSubjects = Object.values(parsed.semesters || {}).flatMap(
                        ...
                        scoredCodes = allSubjects
                            .filter((subj: any) => {
                                ...
                                if (
                                    ['Rớt', 'Hoãn thi', 'Hoàn thành', 'Miễn'].includes(status) ||
                                    ['Rớt', 'Hoãn thi', 'Hoàn thành', 'Miễn'].includes(tk)
                                )
                                    return true;
                                return false;
                            })
                    ...
                    console.warn('Error reading html_score_data:', e);
                }
                ...
                console.log(
                    '[DEBUG] Mã môn từ ICS:',
                    Array.isArray(arr)
                        ? arr.map((subj) => (subj.code || subj.subjectCode || '').toString().toUpperCase().trim())
                        : [],
                );
                console.log(
                    '[DEBUG] Mã môn được thêm vào trạng thái đang học:',
                    filteredArr.map((subj) => (subj.code || subj.subjectCode || '').toString().toUpperCase().trim()),
                );
    ...
    const handleConfirmUpload = async () => {
        ...
        try {
            ...
            const subjectsWithSemester = parsedSubjects
                .filter((subject) => {
            ...
            message.success(
                `Đã upload file lịch học và cập nhật ${subjectsWithSemester.length} môn đang học cho học kỳ ${semesterLabel}!`,
            );
    ...
    return (
        <>
            <Upload beforeUpload={beforeUpload} showUploadList={false} accept=".ics" disabled={!major}>
                ...
                <div className="space-y-4">
                    <div>
                        <p className="text-gray-600 mb-2">
                            Vui lòng kiểm tra và xác nhận thông tin môn học trước khi lưu:
                        </p>
                        <p className="text-sm text-gray-500">
                            Tìm thấy {parsedSubjects.length} môn học mới từ file ICS
                        </p>
                    </div>
